...
选项:
    ...
    -d, --dashboard FILE       仪表板JSON文件 (可选，自动查找)
    -h, --help                 显示此帮助信息
...
find_dashboard_file() {
    if [[ -n "$DASHBOARD_FILE" && -f "$DASHBOARD_FILE" ]]; then
        print_success "使用指定的仪表板文件: $DASHBOARD_FILE"
        return 0
    fi
    ...
    # 可能的仪表板文件位置
    POSSIBLE_FILES=(
        "$PROJECT_ROOT/hygon-dcu-dashboard-simple.json"
        "$PROJECT_ROOT/deploy/monitoring/grafana/dashboards/hygon-dcu-dashboard.json"
        "$SCRIPT_DIR/../monitoring/grafana/dashboards/hygon-dcu-dashboard.json"
        "./hygon-dcu-dashboard-simple.json"
    ...
    for file in "${POSSIBLE_FILES[@]}"; do
        if [[ -f "$file" ]]; then
            DASHBOARD_FILE="$file"
            print_success "找到仪表板文件: $DASHBOARD_FILE"
            return 0
        fi
    done
    
    print_error "未找到仪表板文件"
