...
func (c *HygonSysfsCollector) discoverDevices() {
	...
	for _, entry := range entries {
		...
		if vendor := c.readSysfsFile(vendorPath); vendor != "0x1d94" {
			continue // 不是海光设备
		}
...
func (c *HygonSysfsCollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, device := range c.devices {
		...
		// 功耗信息（优先从hwmon读取，单位是微瓦）
		var avgPower float64
		if device.HwmonPath != "" {
			avgPower = c.readSysfsFloat(filepath.Join(device.HwmonPath, "power1_average"))
			if avgPower > 0 {
				c.avgPower.WithLabelValues(labels...).Set(avgPower / 1000000) // 微瓦转瓦特
			} else {
				// 备用方案：从chip_power_average读取（单位是mW）
				chipPower := c.readSysfsFloat(filepath.Join(device.SysfsPath, "chip_power_average"))
				c.avgPower.WithLabelValues(labels...).Set(chipPower / 1000) // 毫瓦转瓦特
			}
		} else {
			// 备用方案：从chip_power_average读取（单位是mW）
			chipPower := c.readSysfsFloat(filepath.Join(device.SysfsPath, "chip_power_average"))
